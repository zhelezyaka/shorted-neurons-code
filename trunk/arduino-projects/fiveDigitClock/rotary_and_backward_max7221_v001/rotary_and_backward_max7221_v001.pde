...
void columns() {
  for(int col=1;col<4;col++) {
    delay(delaytime);
    rgb.setColumn(0,col,random(255));
    delay(delaytime);
...
void updateDisplay() {
  lc.setColumn(0,1,digMap[((periodCount / 1000) % 10)]);
  lc.setColumn(0,2,digMap[((periodCount / 100) % 10)]);
  lc.setColumn(0,3,digMap[((periodCount / 10) % 10)]);
  lc.setColumn(0,4,digMap[(periodCount % 10)]);
