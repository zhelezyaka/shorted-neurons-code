...
/* works! but too fast:  
  ...
  */
  ...
  lc.setIntensity(1,clockLedsPower);
...
void rows() {
  for(int row=0;row<8;row++) {
    delay(delaytime);
    lc.setRow(1,row,B01110000);
    delay(delaytime);
    lc.setRow(1,row,(byte)0);
/*    for(int i=0;i<row;i++) {
      delay(delaytime);
...
//    delay(delaytime);
    lc.setColumn(1,col,random(random(127),255));
    delay(delaytime);
...
//  rows();
  ...
  lc.setColumn(5,5,B11111111);
  lc.setColumn(6,6,B11111111);
   delay(delaytime*3);
