...
void loop() {
    ...
    if (rf12_recvDone()) {
        ...
        Serial.print(" ");
        Serial.print(float(float(millis())/60000/60));
        Serial.print(" hours, ");
        ...
        if ( blinkmsg) {
          if(rf12_data[5] != MYSELF) {
             ...
             goodPin = false;
             for (int t = 0; t < sizeof(ledPins); t++) {
                   if (actionPin == ledPins[t]) {
                     goodPin = true;
                   }
             }
             if ( !goodPin ) {
               Serial.println("        ERROR: asked for a pin that I don\'t like.");
             } else {
               switch (rf12_data[7]) {
                 default:
                   Serial.println("          ERROR: bad action, needs to be \"H\" or \"l\"!!!!");
                   break;
                 case 'H': 
