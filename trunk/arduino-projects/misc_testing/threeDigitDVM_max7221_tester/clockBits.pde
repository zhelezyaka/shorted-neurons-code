...
//  LEDs.setDigit(0,0,hrs1,false);
    ...
    digits[0] = digMap[mins2];
    digits[1] = (digMap[secs1] | colon1);
    digits[2] = (digMap[secs2] | colon2);
...
/*  Wire.requestFrom(DS3232_I2C_ADDRESS, 2);
  byte reg0E = Wire.receive();
  Serial.print(F("0x0Eh bits requested is:"));
  Serial.println(reg0E, BIN);
  byte reg0F = Wire.receive();
  Serial.print(F("0x0Fh bits requested is:"));
...
//  while(true) {
    ...
    Serial.println(F("setting Alarm flag off"));
    setTo = off;
    ...
    Serial.println(F("setting Alarm flag ON"));
    setTo = on;
