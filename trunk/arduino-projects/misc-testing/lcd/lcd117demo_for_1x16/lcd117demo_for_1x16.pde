...
#define txPin 1 // pin 14 is analog pin 0, on a BBB just use a servo cable :), see Reference pinMode
...
void setup(){
   ...
   mySerial.print("?s4");     // set tabs to six spaces
   delay(1000);               // pause to allow LCD EEPROM to program
...
void loop(){
   ...
   mySerial.print("?x00?y0");              // move cursor to beginning of line 1
   ...
   for (N = 42; N<= 122; N++){             // pick an arbitrary part of ASCII chart - change as you wish
      mySerial.print("?x00?y0");           // locate cursor to beginning of line 1
   ...
   for ( N = 0; N <= 80; N++){         // 16 chars * 5 bits each = 80
      mySerial.print("?y0?x00");       // cursor to beginning of line 1
      delay(10);
   ...
   for ( N = 0; N <= 15; N++){
      ...
      if (N < 8){
         ...
         mySerial.print("?y0?x00?7");       // cursor to beginning of line 1 and writes black character
         mySerial.print("?y0?x00");
         Remainder = (N % 8);               // % sign is modulo operator - calculates remainder
         // now print the remainder
         mySerial.print("?");               // first half of the custom character command
