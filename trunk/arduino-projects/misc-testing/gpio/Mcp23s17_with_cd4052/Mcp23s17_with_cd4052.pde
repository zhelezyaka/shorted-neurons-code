...
void setup()
{
  ...
  digitalWrite(5, LOW);
  pinMode(aMux0, OUTPUT);
  pinMode(aMux1, OUTPUT);
  digitalWrite(aMux0, LOW);
  digitalWrite(aMux1, LOW);
}
...
void loop()
{
  ...
  //{
    ...
    counter=0;
    counter = x1 << 0;
    counter = counter + (x2 << 1);
    counter = counter + (x3 << 2);
    counter = counter + (x4 << 3);
