...
void setup()
{
  ...
  digitalWrite(joyBtn, HIGH); //turn on pullup
...
//  tempLoop();
  ...
  int x=abs(1024 - analogRead(joyX));
  int y=analogRead(joyY);
  int btnState=digitalRead(joyBtn);
  if (temp <80 ) {
    str << rtcString << " uptime=" << ts << " sec, x=" << x << ", y=" << y << ", btn=" << btnState << ", i=" << iterations << ", " << temp << "F, trig=ON" << endl; 
    //digitalWrite(actLed, HIGH); 
  } else {
    str << rtcString << " uptime=" << ts << " sec, x=" << x << ", y=" << y << ", btn=" << btnState << ", i=" << iterations << ", " << temp << "F, trig=off" << endl; 
    //digitalWrite(actLed, LOW); 
