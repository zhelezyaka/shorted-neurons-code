...
boolean brightEnough() {
  ...
  if (okToShoot && (photoLevel < lowLightLevel)) {
    ...
    Serial.print("brightEnough: state change from OK to tooDark");
    okToShoot = false;
  } else {
    if ((! okToShoot) && (photoLevel > highLightLevel )) {
      // changing from tooDark back to OK
      Serial.print("brightEnough: state change from tooDark to OK : ");
      okToShoot = true;
    } else {
      Serial.print("brightEnough: state did NOT change, light level is: ");
    }
  }
...
//  tempLoop();
  ...
  if (temp <80 ) {
    str << rtcString << " uptime=" << ts << " sec, batt=" << bv << ", i=" << iterations << ", " << temp << "F is under 80, turning the \"heater\" on" << endl; 
    //digitalWrite(actLed, HIGH); 
  } else {
    str << rtcString << " uptime=" << ts << " sec, batt=" << bv << ", i=" << iterations << ", " << temp << "F is >=80, turning the \"heater\" OFF" << endl; 
    //digitalWrite(actLed, LOW); 
