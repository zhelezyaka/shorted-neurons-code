...
//    delay(2);               // wait until the last serial character is send
    ...
    Serial << F("_______________before for loop in sleepWithBeacon, Beacon's i=") << i << "out of " << dur << endl;

    for ( i=0; i<=dur; i++) {
      //Serial << F("just inside for loop in sleepWithBeacon, Beacon's i=") << i << "out of " << dur << endl;
      ...
      Serial << F("going to sleep, Beacon's i=") << i << "out of " << dur << endl;
...
//  }
  Serial.print(F("EXITING sleepWithBeacon() at the end of the loop... not sure we should ever get here! "));
}
...
#define warmupTime 6000
#define modeChangeTime 3000
#define modeHoldTime 1000
...
*/
    ...
    //go to pic mode
    Serial.println(F("s004:setPicMode"));
    digitalWrite(BLUE, HIGH);
    digitalWrite(shutterSw, LOW);
    delay(modeHoldTime);
    digitalWrite(shutterSw, HIGH);
    delay(modeChangeTime);
...
////////////////////////////
    ...
    if (powerOff) {
      ...
      digitalWrite(powerSw, HIGH);
      pinMode(powerSw, INPUT);
      digitalWrite(powerSw, LOW);  // let input float again... cam likes it apparently
      leftOff = true;
    } else {
...
{
  iterations++;
  Serial << F("top of loop, iteration= ") << iterations << endl;
  ...
  //chkMem();
  if (temp < -273 ) {
    Serial.println(F("temperatur reading horked"));
    digitalWrite(actLed, LOW); 
  }
  
  Serial << F("calling rtcTest from main loop") << endl;
  rtcTest();
  ...
  sleepWithBeacon(rtcPicPeriod * 60 * 2);  // approx seconds
