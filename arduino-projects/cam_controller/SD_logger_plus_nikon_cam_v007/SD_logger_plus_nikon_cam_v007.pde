...
#define lowLightLevel 600
#define highLightLevel 700
...
void shoot() {
  if (okToShoot) {
    ...
    if (powerOff) {
      ...
      Serial.println("s006:fast,NOTcamOff");
      leftOff = false;
    }
  } else {
     Serial.println("s008=noshot");
  }  
  Serial.println("s007:done!");
...
boolean brightEnough() {
  ...
  if (okToShoot && (photoLevel < lowLightLevel)) {
    ...
    Serial.print("bright: OK->tooDark");
    okToShoot = false;
  } else {
    if ((! okToShoot) && (photoLevel > highLightLevel )) {
      // changing from tooDark back to OK
      Serial.print("bright: tooDark->OK");
      okToShoot = true;
    } else {
      Serial.print("bright: no change");
    }
  }
