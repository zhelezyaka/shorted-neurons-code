...
{
  SPI.begin();
  SPI.setClockDivider(SPI_CLOCK_DIV32);  // 2MHz SPI clock if 16MHz system
  pinMode(5, OUTPUT);     
  pinMode(6, OUTPUT);       
  pinMode(3, OUTPUT);      
  ...
  digitalWrite(4, LOW);
  delay(10);
    digitalWrite(4, HIGH);  // effectively enable the mcp23s17, dont do earlier as we dont want its spurious/random outputs turned on
  delay(10);
  digitalWrite(4, LOW);
  delay(10);
    digitalWrite(4, HIGH);  // effectively enable the mcp23s17, dont do earlier as we dont want its spurious/random outputs turned on
  ...
  digitalWrite(5, LOW);   // set the LED on
  digitalWrite(6, HIGH); // turn on HV rail
  
  // Set all pins to be outputs (by default they are all inputs)
  Mcp23s17B.pinMode(OUTPUT);
  // Change all pins at once, 16-bit value
  Mcp23s17B.port(0x0F0F);
  // Write to individual pins
  Mcp23s17B.digitalWrite(8,LOW);
  Mcp23s17B.digitalWrite(12,HIGH);
  // Read all pins at once, 16-bit value
  uint16_t pinstateB = Mcp23s17B.port();
  // Set pin 14 (GPIO B6) to be an input
  //Mcp23s17B.pinMode(14,INPUT);
  // Read the value of pin 14
  //Mcp23s17B.digitalRead(14);
  
  
}

// Cycle the output lines at a staggered rate, pin/2 milliseconds
long timeoutInterval = 50;
...
void timeout()
{
  ...
  //delay(500);
  digitalWrite(6, LOW); // turn on HV rail
  delay(10000);
  digitalWrite(6, HIGH); // turn on HV rail
  delay(10000);
...
void loop()
{
  ...
  digitalWrite(5, HIGH);   // set the LED on
  digitalWrite(6, HIGH);
  delay(random(10,200));              // wait for a second
