...
// end bryan mods except for calls to digitalWrite
...
/* FIXME bts commented out cause we cant set pinmodes on register
...
void LiquidCrystal::begin(uint8_t cols, uint8_t lines, uint8_t dotsize) {
  ...
  digitalWriteShifted(_rs_pin, LOW);
  digitalWriteShifted(_enable_pin, LOW);
  if (_rw_pin != 255) { 
    digitalWriteShifted(_rw_pin, LOW);
  }
...
void LiquidCrystal::send(uint8_t value, uint8_t mode) {
  digitalWriteShifted(_rs_pin, mode);
...
void LiquidCrystal::pulseEnable(void) {
  digitalWriteShifted(_enable_pin, LOW);
  delayMicroseconds(1);    
  digitalWriteShifted(_enable_pin, HIGH);
...
void LiquidCrystal::write4bits(uint8_t value) {
  for (int i = 0; i < 4; i++) {
    //bts pinMode(_data_pins[i], OUTPUT);
    digitalWriteShifted(_data_pins[i], (value >> i) & 0x01);
  }
