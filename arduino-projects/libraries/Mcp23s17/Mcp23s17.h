...
class MCP23S17
{
  public:
    ...
    void pinMode(uint16_t value);
    void setPullups(uint16_t value);
    void setAllInputPolarity(uint16_t value);
    void port(uint16_t value);
    uint16_t port();

    // Work on individual pins 0-15
    void pinMode(uint8_t pin, bool mode);
    void setPullup(uint8_t pin, bool mode);
  ...
  protected:
    ...
    const static uint8_t IOPOLA = 0x02;
    const static uint8_t IOPOLB = 0x03;
    const static uint8_t IOPOL  = IOPOLA;
