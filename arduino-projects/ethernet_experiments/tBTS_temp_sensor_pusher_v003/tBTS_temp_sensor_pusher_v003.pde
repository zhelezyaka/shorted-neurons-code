...
byte mac[] = { 0xBB, 0xAD, 0xBE, 0xEF, 0xFE, 0xED };
byte ip[] = { 192, 168, 1, 221 };
byte gateway[] = { 192, 168, 1, 1 };
//byte server[] = { 71, 196, 146, 152 }; // www.bluefx.com
byte server[] = { 192, 168, 1, 12 }; // www.bluefx.com
...
void loop()
{
  ...
  if (digitalRead(4)) { 
    ...
    if (client.connect()) {
      ...
      Serial.print("turn on red switch to attempt a reset\n");
      while (! digitalRead(5)) {
        delay(2000);    
      }
      Serial.print("attempting a client.stop...");
      client.stop();
      Serial.print(" done\n");
      Serial.print("Attempting to continue normally....\n");
    }
  }  else {
    Serial.println("press the button to post a temp...");
    delay(10000);
  }
  delay(10);
